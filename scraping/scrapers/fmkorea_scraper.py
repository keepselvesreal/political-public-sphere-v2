...
주요 기능:
- FMKoreaScraper: 정식 스크래퍼 클래스 (line 35-80)
- scrape_post_detail: 개별 게시글 상세 스크래핑 (line 82-150)
...
작성자: AI Assistant
작성일: 2025-01-28
최종 수정: 2025-01-28 (실험용에서 정식으로 승격)
목적: FM코리아 게시글 정밀 스크래핑 및 원본 재현
"""
...
class FMKoreaScraper:
    ...
    async def scrape_post_detail(self, post_url: str) -> Dict:
        """
        정식 게시글 상세 스크래핑 (기존 스크래퍼와 동일한 방식)
        ...
        Returns:
            Dict: 정식 게시글 데이터 (순서 보존)
        """
        try:
            logger.info(f"🧪 정식 게시글 스크래핑 시작: {post_url}")
            ...
            logger.info(f"✅ 정식 게시글 스크래핑 완료: {len(content_list)}개 콘텐츠 요소")
            return experiment_data
            
        except Exception as e:
            logger.error(f"💥 정식 게시글 스크래핑 실패: {e}")
            return {
                'post_id': self.parse_post_id_from_url(post_url),
                'post_url': post_url,
                'scraped_at': datetime.now(self.kst).isoformat(),
...
async def scrape_multiple_posts_experiment(post_urls: List[str]) -> List[Dict]:
    """
    여러 게시글 정식 스크래핑 편의 함수
    
    Args:
        post_urls: 게시글 URL 리스트
    
    Returns:
        List[Dict]: 정식 스크래핑 결과 리스트
    """
    results = []
    ...
    async with FMKoreaScraper() as scraper:
        for url in post_urls:
            try:
                result = await scraper.scrape_post_detail(url)
