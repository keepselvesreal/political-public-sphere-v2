...
FMKOREA_SELECTORS = {
    ...
    "comments": {
        ...
        # 댓글 레벨 (margin-left 스타일에서 추출)
        "level": None,
        # 대댓글 여부 (re 클래스 확인)
        "is_reply": None,
        # 부모 댓글 ID (findParent 링크에서 추출)
...
async def extract_text_content(element: ElementHandle) -> str:
    """요소에서 텍스트 추출 (개선된 버전 - 줄바꿈 및 스타일 보존)"""
    try:
        # HTML 내용을 가져와서 줄바꿈과 스타일 정보 보존
        html_content = await element.evaluate("""
            element => {
                // 줄바꿈 태그들을 실제 줄바꿈으로 변환
    ...
    async def extract_comments(self, page: Page) -> List[Dict]:
        ...
        try:
            ...
            for i, element in enumerate(comment_elements):
                ...
                # 댓글 레벨 계산 (margin-left 스타일에서)
                style_attr = await extract_attribute(element, "style")
                level = 1  # 기본 레벨
                if style_attr and "margin-left:" in style_attr:
                    # margin-left:2% -> 레벨 2, margin-left:4% -> 레벨 3, ...
                    margin_match = re.search(r'margin-left:\s*(\d+)%', style_attr)
                    if margin_match:
                        margin_percent = int(margin_match.group(1))
                        level = (margin_percent // 2) + 1  # 2% -> 2레벨, 4% -> 3레벨, ...
                
                comment_data["level"] = level
                
                # 대댓글 여부 확인 (re 클래스 확인)
                class_attr = await extract_attribute(element, "class")
                is_reply = "re" in class_attr if class_attr else False
                comment_data["is_reply"] = is_reply
                
                # 부모 댓글 ID 추출 (findParent 링크에서)
                parent_comment_id = ""
                if is_reply:
                    # findParent 링크 찾기
                    find_parent_element = await element.query_selector("a.findParent")
                    if find_parent_element:
                        onclick_attr = await extract_attribute(find_parent_element, "onclick")
                        if onclick_attr:
                            # onclick="return findComment(8478043539);" 에서 ID 추출
                            parent_match = re.search(r'findComment\((\d+)\)', onclick_attr)
                            if parent_match:
                                parent_comment_id = f"comment_{parent_match.group(1)}"
                
                comment_data["parent_comment_id"] = parent_comment_id
                    
                # 댓글 내용 추출 (여러 셀렉터 시도)
                content_text = ""
                content_selectors = [".comment-content .xe_content", ".xe_content", "div[class*='xe_content']"]
                ...
                # findParent 링크 텍스트 제거 (대댓글에서 "@작성자명" 부분 제거)
                if content_text and is_reply:
                    # "@작성자명" 패턴 제거
                    content_text = re.sub(r'^[^가-힣]*[가-힣]+\s*', '', content_text).strip()
                ...
                if comment_data["content"].strip():
                    ...
                    print(f"  ✅ 댓글 {len(comments)} (레벨{comment_data['level']}): {comment_data['author']} - {comment_data['content'][:30]}...")
                    if comment_data["is_reply"]:
                        print(f"      └─ 대댓글 (부모: {comment_data['parent_comment_id']})")
                
        except Exception as e:
            print(f"댓글 추출 오류: {e}")
            
        print(f"📝 총 {len(comments)}개 댓글 추출 완료 (관계 파악 포함)")
        return comments
